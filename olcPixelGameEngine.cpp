...
namespace olc
{
	...
	{ // Thanks to IanM-Matrix1 #PR121
		...
		if (radius > 0)
		{
			...
			{
				// Draw even octants
				if (mask & 0x01) Draw(x + x0, y - y0, p);// Q6 - upper _right _right
				if (mask & 0x04) Draw(x + y0, y + x0, p);// Q4 - lower lower _right
				if (mask & 0x10) Draw(x - x0, y + y0, p);// Q2 - lower _left _left
				if (mask & 0x40) Draw(x - y0, y - x0, p);// Q0 - upper upper _left
				if (x0 != 0 && x0 != y0)
				{
					if (mask & 0x02) Draw(x + y0, y - x0, p);// Q7 - upper upper _right
					if (mask & 0x08) Draw(x + x0, y + y0, p);// Q5 - lower _right _right
					if (mask & 0x20) Draw(x - y0, y + x0, p);// Q3 - lower lower _left
					if (mask & 0x80) Draw(x - x0, y - y0, p);// Q1 - upper _left _left
				}
...
#pragma warning disable format
			...
			// cache for reuse
			Module._olc_EmscriptenShellCss = "width: 100%; height: 70vh; margin-_left: auto; margin-_right: auto;";
			...
			Module.canvas.parentNode.addEventListener("resize", function(e) {
				...
				if (document.fullscreenElement != null)
				{
					...
					var _left = (e.detail.width - viewWidth) / 2;
					Module.canvas.style.position = "fixed";
					Module.canvas.style.top = top.toString() + "px";
					Module.canvas.style._left = _left.toString() + "px";
